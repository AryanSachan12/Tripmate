...
function TripPageContent() {
  ...
  const fetchTripData = async (id) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        console.error('Failed to load trip:', response.error);
      }
    } catch (err) {
  ...
  const handleTripCreated = (newTrip) => {
    console.log('📥 handleTripCreated called with:', newTrip);
    console.log('🆔 Trip ID received:', newTrip?.id);
    
    setCurrentTrip(newTrip);
    
    // Update URL to remove create mode and add trip ID
    const newUrl = `/trip?id=${newTrip.id}`;
    console.log('🔗 Updating URL to:', newUrl);
    window.history.replaceState({}, '', newUrl);
    
    console.log('✅ URL updated, current trip set');
  };
