...
export default function ExplorePage() {
  ...
  // Function to extract unique locations from trips
  const extractAvailableLocations = (tripsData) => {
    const locationSet = new Set();
    
    tripsData.forEach(trip => {
      // Try to get location from different possible fields
      const location = trip.primary_destination || trip.location || trip.destination;
      if (location && location.trim()) {
        locationSet.add(location.trim());
      }
      
      // Also check cities if available
      if (trip.cities && trip.cities.length > 0) {
        trip.cities.forEach(city => {
          if (city.city_name && city.city_name.trim()) {
            locationSet.add(city.city_name.trim());
          }
        });
      }
    });
  ...
  const loadTrips = async () => {
    try {
      ...
      if (response.success) {
        ...
        // Extract available locations from trips
        setAvailableLocations(extractAvailableLocations(tripsWithCities));
      } else {
  ...
  const handleCopyTrip = (trip) => {
    ...
    const copyData = {
      title: `Copy of ${trip.title || 'Untitled Trip'}`,
      description: trip.description || '',
      cities: trip.cities ? trip.cities.map(city => ({
        id: city.id || Date.now(),
        city_name: city.city_name || city.destination || city.location || '',
  ...
  const filteredTrips = trips.filter(trip => {
    ...
    // Location filter - check if any of the trip's locations match the selected filter
    const matchesLocation = !selectedFilters.location || 
                           (trip.primary_destination && trip.primary_destination.includes(selectedFilters.location)) ||
                           (trip.location && trip.location.includes(selectedFilters.location)) ||
                           (trip.destination && trip.destination.includes(selectedFilters.location)) ||
                           (trip.cities && trip.cities.some(city => 
                             city.city_name && city.city_name.includes(selectedFilters.location)
                           ));
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-16 sm:pt-20">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-8">
        {/* Hero Section */}
        <div className="relativemb-8 sm:mb-12">
            ...
            <div className="absolute inset-0 flex flex-col justify-center items-start p-6 sm:p-8 lg:p-12">
              <div className="max-w-2xl">
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                  ...
                  <Link href="/trip" className="bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white border border-white/30 px-6 sm:px-8 py-3 sm:py-4 rounded-xl font-semibold text-base sm:text-lg transition-all duration-200">
                    Create Trip
                  </Link>
                </div>
              </div>
              ...
              filteredTrips.map(trip => (
                <div key={trip.id} className="bg-white rounded-xl sm:rounded-2xl shadow-sm hover:shadow-lg transition-shadow overflow-hidden">
                  <div className="relative h-40 sm:h-48">
                    <Image
                      src={trip.cover_image_url || "/placeholder.png"}
                      alt={trip.title}
                      fill
                      className="object-cover"
                    />
